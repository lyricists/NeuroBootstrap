...
class EEG_augmentation:
    ...
    def augData(self):
        ...
        for n in tqdm(range(len(self.subIdx))):
            ...
            for k in range(self.kfold):
                ...
                # ----- Test data -----
                tmpPos, tmpNeg = [], []
                for _ in range(int(self.Trial_num * 0.2)):
                    rIdx = np.random.choice(
                        len(self.split_data[n][k]["test_positive"]), self.avg_num
                    )
                    tmpPos.append(
                        np.mean(
                            self.pcaDataset[k]["pcaData"][
                                :, :, self.split_data[n][k]["test_positive"][rIdx], n
                            ],
                            axis=2,
                        )
                    )
                    ...
                    rIdx = np.random.choice(
                        len(self.split_data[n][k]["test_negative"]),
                        self.avg_num,
                    )
                    tmpNeg.append(
                        np.mean(
                            self.pcaDataset[k]["pcaData"][
                                :, :, self.split_data[n][k]["test_negative"][rIdx], n
                            ],
                            axis=2,
                        )
                    )
                ...
                foldData.append({"TestData": TestData})
    ...
    def saveData(self):
        ...
        with open(self.fPath + "test/" + self.saveName, "wb") as file:
...
if __name__ == "__main__":
    ...
    for TOI in TOI_list:
        saveName = f"AugEEG_test_{TOI}_16.pkl"
