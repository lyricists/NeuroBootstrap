...
def main(args):
    print(
        ...
        f"(Train: {args.trainTOI}, Test: {args.testTOI}, 80/10/10 split) ==="
    )

    hyper = get_hparams(args.model)

    # Construct output filename
    save_name = f"{args.model}_{args.trainTOI}_{args.testTOI}_ablation.pkl"
...
if __name__ == "__main__":
    ...
    p.add_argument("--trainTOI", type=str, required=True)
    p.add_argument("--testTOI", type=str, required=True)
    args = p.parse_args()
